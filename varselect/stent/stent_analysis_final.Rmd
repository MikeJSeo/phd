...
```{r, eval = FALSE}
# aa[aa[,1] != 0,1]
# multivessel disease: 1.0778950, ladtreated = 1: 1.0691515, m_dia less than 3: -5.3789130, num_stents = 5 (3.1705418)
mean1 <- c(1, 1.0778950, 1.0691515, -5.3789130, 3.1705418) %*% c(-0.074123784, -0.003832965, -0.095091303, 0.025749350, -0.044570843)
exp(mean1)

# no multivessel disease: -0.9276506,ladtreated=0: -0.9352369, m_dia_above_3: 0.1858944, num_stents = 1: -0.6300019
mean2 <- c(1, -0.9276506, -0.9352369, 0.1858944, -0.6300019) %*% c(-0.074123784, -0.003832965, -0.095091303, 0.025749350, -0.044570843)
exp(mean2)
```
...
```{r, eval = FALSE}
# 80 years: 0.9325098, female(gender = 0): -1.6474649, diabetes=1: 1.8095668, stable_cad=0: -0.7168387, multivessel=1: 1.0778950, ladtreated = 1: 1.0691515, overlap=1: 2.348039, m_dia_above_3=0: -5.3789130, num_stents = 5 (3.1705418)
mean1 <- c(1, 0.9325098, -1.6474649, 1.8095668, -0.7168387, 1.0778950, 1.0691515, 2.348039, -5.3789130, 3.1705418) %*% coef(cvfit, s = "lambda.min")[18:27]
exp(mean1)

# 50 years: -1.522422, male(gender =1): 0.606939, diabetes=0: -0.5525687, stable_cad=1: 1.3948883, multivessel=0: -0.9276506, ladtreated=0: -0.9352369, overlap=0: -0.425849, m_dia_above_3=1: 0.1858944, num_stents = 1: -0.6300019
mean2 <- c(1, -1.522422, 0.606939, -0.5525687, 1.3948883, -0.9276506, -0.9352369, -0.425849, 0.1858944, -0.6300019) %*% coef(cvfit, s = "lambda.min")[18:27]
exp(mean2)
```
...
model_bayesLASSO<- jags(data = data_jags_stent, inits = NULL,
                 parameters.to.save =  c("gamma", "delta", "beta"),
...
```{r, eval = FALSE}
# 80 years: 0.9325098, female(gender = 0): -1.6474649, diabetes=1: 1.8095668, stable_cad=0: -0.7168387, multivessel=1: 1.0778950, ladtreated = 1: 1.0691515, overlap=1: 2.348039, m_dia_above_3=0: -5.3789130, num_stents = 5: 3.1705418
mat1 <- do.call(rbind, model_bayesLASSO[,c("delta[2]", "gamma[1]", "gamma[2]", "gamma[3]", "gamma[4]", "gamma[5]", "gamma[6]", "gamma[7]", "gamma[8]", "gamma[9]")])
...
```{r}
# 80 years: 0.9325098, female(gender = 0): -1.6474649, diabetes=1: 1.8095668, stable_cad=0: -0.7168387, multivessel=1: 1.0778950, ladtreated = 1: 1.0691515, overlap=1: 2.348039, m_dia_above_3=0: -5.3789130, num_stents = 5: 3.1705418
mat1 <- do.call(rbind, model_SSVS[,c("delta[2]", "gamma[1]", "gamma[2]", "gamma[3]", "gamma[4]", "gamma[5]", "gamma[6]", "gamma[7]", "gamma[8]", "gamma[9]")])
sum1 <- mat1 %*% c(1, 0.9325098, -1.6474649, 1.8095668, -0.7168387, 1.0778950, 1.0691515, 2.348039, -5.3789130, 3.1705418)
