...
```{r}
niter <- 100 #number of simulation to run
...
```{r simple_null_scenario1, cache = TRUE, results = "hide"}
simple_null_store_mse1 <- matrix(NA, nrow = niter, ncol = 3)
simple_null_store_sd1 <- rep(NA, niter)
...
for(i in seq(niter)){
  ...
  simple_null_store_mse1[i,] <- find_performance2(mse_null, correct_em_values, correct_em)
  simple_null_store_sd1[i] <- summary(m1)$coef["treat", "Std. Error"]
}
simple_null_store_mse1_mean <- apply(simple_null_store_mse1, 2, mean)
simple_null_store_sd1_mean <- mean(simple_null_store_sd1)
```


```{r simple_null_scenario2, cache = TRUE, results = "hide"}
simple_null_store_mse2 <- matrix(NA, nrow = niter, ncol = 3)
simple_null_store_sd2 <- rep(NA, niter)
