...
```{r}
#function to cross validate bayesian LASSO
cv.bayesLasso <- function(data, model.type, X, lambda = seq(20,1,by = -1)){
  ...
  for (i in 1:kk)
  {
    ...
    model <- jags.model("IPD-MA-bayesLASSO.txt", data = data_jags, n.adapt = 100)
    samples <- coda.samples(model, variable.names = c("alpha", "beta", "delta", "g", "tau"), 
                            n.chains = 2, n.update = 200, n.iter = 2000)
...
#lambda.min <- cv.bayesLasso(mydata, model.type = "binary", X = X)
