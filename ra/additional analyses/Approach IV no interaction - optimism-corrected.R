...
prediction_SCQM <- findPrediction2(SCQM, result)
prediction_BSRBR <- findPrediction2(BSRBR, result)
...
for(ii in 1:200){
  ...
  samples_SCQM_bootstrap <- firstStage(SCQM_bootstrap, "first stage-nointeraction.txt", mm = 1, index = c("a", "b", "d", "sigma"), no.interaction = TRUE)
  r2 <- summarize_each_study(samples_SCQM_bootstrap)
  r2 <- unstandardize_coefficients2(r2, SCQM_bootstrap)
  
  y <- list(y1 = r1[[1]], y2 = r2[[1]], y3 = r3[[1]], y4 = r4[[1]], y5 = r5[[1]])
  Sigma <- list(Sigma1 = r1[[2]], Sigma2 = r2[[2]], Sigma3 = r3[[2]], Sigma4 = r4[[2]], Sigma5 = r5[[2]])
  result <- secondStage(y = y, Sigma = Sigma, jags_file = "second stage-ApproachIV-nointeraction.txt", no.interaction = TRUE, n.iter = 10000)
 
  prediction_SCQM_bootstrap <- findPrediction2(SCQM_bootstrap, result)
  performance_SCQM_bootstrap <- findPerformance(prediction_SCQM_bootstrap)
  
  prediction_SCQM_test <- findPrediction2(SCQM, result)
...
for(ii in 1:200){
  ...
  samples_BSRBR_bootstrap <- firstStage(BSRBR_bootstrap, "first stage-nointeraction.txt", mm = 1, index = c("a", "b", "d", "sigma"), no.interaction = TRUE)
  r1 <- summarize_each_study(samples_BSRBR_bootstrap)
  r1 <- unstandardize_coefficients2(r1, BSRBR_bootstrap)
  
  y <- list(y1 = r1[[1]], y2 = r2[[1]], y3 = r3[[1]], y4 = r4[[1]], y5 = r5[[1]])
  Sigma <- list(Sigma1 = r1[[2]], Sigma2 = r2[[2]], Sigma3 = r3[[2]], Sigma4 = r4[[2]], Sigma5 = r5[[2]])
  result <- secondStage(y = y, Sigma = Sigma, jags_file = "second stage-ApproachIV-nointeraction.txt", no.interaction = TRUE, n.iter = 10000)
  
  prediction_BSRBR_bootstrap <- findPrediction2(BSRBR_bootstrap, result)
  performance_BSRBR_bootstrap <- findPerformance(prediction_BSRBR_bootstrap)
  
  prediction_BSRBR_test <- findPrediction2(BSRBR, result)
