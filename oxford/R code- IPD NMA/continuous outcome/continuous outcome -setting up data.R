...
aggregate(data_1$HAMD_WEEK_0, list(data_1$STUDYID), mean)
aggregate(data_1$AGE, list(data_1$STUDYID), mean)


# check imputed datasets
imp1=5
study=1
dat1<-study.data[[study]][study.data[[study]]$imp==imp1 ,]
dat1$baseline=(dat1$baseline-20)/4
dat1$age=(dat1$age-45)/15
dat1$y=(dat1$y-6)/5
lm(y~treat*(baseline+age+sex+HAMD_3+HAMD_4+HAMD_6+HAMD_10+HAMD_11+HAMD_13+HAMD_17), data=dat1)
