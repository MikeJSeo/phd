...
#lm1=lm(y~treat.coded*(baseline+age+sex+HAMD_3+HAMD_4+HAMD_6+HAMD_10+HAMD_11+HAMD_13+HAMD_17), data=dat2)
...
ctreat~ dnorm(0,0.01)
ctreat.age~ ddexp(0, ttt)
ctreat.sex~ ddexp(0, ttt)
th3~ ddexp(0, ttt)
th4~ ddexp(0, ttt)
th6~ ddexp(0, ttt)
th10~ ddexp(0, ttt)
...
th13~ ddexp(0, ttt)
th17~ ddexp(0, ttt)
ctreat.baseline~ ddexp(0, ttt)


ttt <- lambda * prec  # uses conditional prior
...
jags.m=0
jags.m <- jags.model(model1.spec, data = data, n.chains =4, n.adapt = 3000) ### increase!
...
closeAllConnections()
samps<- coda.samples(jags.m, params, n.iter =10000) ### increase!
...
#sqrt(diag((N %*% v1 %*% t(N))[12:22,12:22]))
print(i)
}
...
for(i in 1:2){
ttt[i] <- lambda[i] * prec  # uses conditional prior
