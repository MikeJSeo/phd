...
for(i in 2:Nt){
 ...
 results[i,k]= round((mean.estimates.nma[which.i]-mean.estimates.nma[which.k]) %*% new.patient,2)
 ...
 SE[i,k]=round(sqrt(new.patient.var %*% var1 %*% new.patient.var), digits=2)
...
## all treatments vs. placebo   -- these are log(odds ratios)!
