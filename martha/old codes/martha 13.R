...
legend(2, 0, c("0.1 > p > 0.05", "0.05 > p > 0.01", "< 0.01"),   fill = c("green", "yellow", "pink"), bty = "n")
...
print(paste("Global test for inconsistency, p-value ",round(net1$pval.Q.inconsistency,5)," (Q=",round(net1$Q.inconsistency),",d.o.f.",net1$df.Q.inconsistency, ")",sep=""))

options(digits=3)
forest(net1, sortvar = -pscore, reference.group = "placebo",
       label.right = "Favours placebo", label.left = "Favours drug", digits=2, xlim=c(0.1,15))

### draw prediction interval
plot.pred(net1, outcome = outcome, xlow=0.1,xhigh=12)

###create league table
league.table=netleague(net1, digits=2)
l1=as.matrix(league.table$random)
diag(l1)=toupper(substr(diag(l1),1,4))
write.xlsx(l1,file=paste(outcome,".xlsx"), sheetName = "league")  
netrank=netrank(net1)

write.xlsx(as.matrix(netrank$Pscore.random),file=paste(outcome,".xlsx"), sheetName = "ranks", append = T)  
print(netrank,comb.fixed = F, digits=2)

options(digits=2)
options(scipen=100)
options(max.print = 3000)
#print(netsplit(net1), digits = 2,backtransf = TRUE, comb.fixed = FALSE)
split=netsplit(net1)
...
ind.pla=which(colnames(net1$TE.random)=="placebo")
options(digits=3)
...
for( i in 1:length(OR.pla$drug)){
  ind=which(colnames(net1$TE.random)==OR.pla$drug[i])
  if(ind<ind.pla){
    OR.pla$logOR[i]=-net1$TE.random[ind.pla,ind]
    OR.pla$seTE[i]=net1$seTE.predict[ind.pla,ind] #changed to seTE.predict
  }
  if(ind>ind.pla){
    OR.pla$logOR[i]=-net1$TE.random[ind.pla,ind]
    OR.pla$seTE[i]=net1$seTE.predict[ind.pla,ind] #changed to seTE.predict
...
# add placebo and draw forest plot

## this is with placebo
#metafor::forest(c(rate.pla, event.rates$event.rate), ci.lb = c(lb.pla, event.rates$CI.lower),
 #               ci.ub = c(ub.pla, event.rates$CI.upper), slab = c("placebo", as.character(event.rates$drug)), xlab = "Event rates")

## this is without placebo
metafor::forest(c(event.rates$event.rate), ci.lb = c(event.rates$CI.lower),
               ci.ub = c(event.rates$CI.upper), slab = c(as.character(event.rates$drug)),
               xlab = "Event rates",xlim=c(-0.15,0.5), psize=0.8, refline=rate.pla, digits=3)
...
for( i in 1:length(OR.pla$drug)){
  ind=which(colnames(net1$TE.random)==OR.pla$drug[i])
  if(ind<ind.pla){
    OR.pla$seTE[i]=net1$seTE.random[ind.pla,ind] #changed to seTE.random
  }
  if(ind>ind.pla){
    OR.pla$seTE[i]=net1$seTE.random[ind.pla,ind] #changed to seTE.random
  }
...
write.xlsx(OR.pla2,file=paste(outcome,".xlsx"), sheetName = "Z.scores", append = T)  
...
outcome="BLOOD PRESSURE DECREASED"
data3.6=p3=descr.dose=net3=mL=NULL
data1 <- split_data(original,outcome ) 
data1=data1[data1$Dose_range=="Licensed",]
data2=remove.onearm(data1)
data2=data2[data2$Dosing_schedule=="Fixed",]
problem_events=which(data2$No_randomised<data2$martha_outcome)
data2=data2[data2$No_randomised>=data2$martha_outcome,];
data3 = data2
data3= harmonize_baseline(data3)
...
p3$comparison=paste(p3$Drug1, "-", p3$Drug2)
comparison.list=unique(p3$comparison)
descr.dose=data.frame("comparison"=comparison.list,"studies"=NA)

for(i in 1:length(descr.dose$comparison)){
  descr.dose$studies[i]=sum(p3$comparison==descr.dose$comparison[i])
 }
...
#p3=p3[p3$treat1!="citalopram_L"&p3$treat1!="escitalopram_L",]
...
forest(mL, backtransf=T, overall=F, hetlab = "", print.I2 =F,print.tau2 = F , 
       ...
       label.right = "Favours placebo",label.left = "Favours low dose", xlim=c(0.1,30))
...
forest(mH, backtransf=T, overall=F, hetlab = "", print.I2 =F,print.tau2 = F , 
       leftlabs=c("Drug",NA,NA), print.pval.Q =F, leftcols=c("Drug"),
       label.right = "Favours placebo",label.left = "Favours high dose", xlim=c(0.2,20))
...
forest(mHL, backtransf=T, overall=F, hetlab = "", print.I2 =F,print.tau2 = F , 
       leftlabs=c("Drug",NA,NA), print.pval.Q =F, leftcols=c("Drug"),
       label.right = "Favours low dose",label.left = "Favours high dose", xlim=c(0.2,30))
