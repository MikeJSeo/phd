...
meta.pla = metaprop(event = round(data3$martha_outcome[data3$Drug=="placebo"]), n = data3$No_randomised[data3$Drug=="placebo"], prediction = TRUE, method = "inverse")
rate.pla = exp(meta.pla$TE.random) 
lb.pla = exp(meta.pla$TE.random - 1.96 * meta.pla$seTE.predict)
ub.pla = exp(meta.pla$TE.random + 1.96 * meta.pla$seTE.predict)
