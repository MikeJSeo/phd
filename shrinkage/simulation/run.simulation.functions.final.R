...
run.simulation <- function(){
  ...
  for(i in seq(niter)){
    ...
    set.seed(i)
    data <-generate.simulation(Nstudies = Nstudies, Ncovariate = Ncovariate, continuous.cov = continuous.cov, pf = pf, em = em, beta = beta, gamma = gamma, model.type = model.type, tau = tau, t_dist = t_dist, sampleSize = sampleSize)
...
#adaptive LASSO
run.simulation2.1 <- function(){
  adapt_store_mse <- matrix(NA, nrow = niter, ncol = 4)
  adapt_store_sd <- matrix(NA, nrow = niter, ncol = 3)
  adapt_store_bias <- matrix(NA, nrow = niter, ncol = 3)
  ...
  for(i in seq(niter)){
    ...
    cvfit <- cv.glmnet(as.matrix(data_glmnet[,-1]), as.matrix(data_glmnet[,1]), penalty.factor = p.fac2, family = family, standardize = FALSE, type.measure = "deviance", alpha = 1, lambda = lambdas)  
    aa <- coef(cvfit, s = "lambda.min")

    mean_values <-  sapply(col_labels, function(x) ifelse(x %in% rownames(aa)[aa[,1] != 0], aa[x,1], 0))
    adapt_store_mse[i,] <- find_performance(mean_values, correct_em_values, correct_em, data_glmnet[,col_labels])
  }
  adapt_store_mse_mean <- apply(adapt_store_mse, 2, mean)

  result_matrix_mse <- matrix(NA, nrow = 1, ncol = 4)
  colnames(result_matrix_mse) <- c("false em mse", "true em mse","treatment mse", "patient specific trt mse")
  rownames(result_matrix_mse) <-  c("adpative lasso")
  result_matrix_mse[1,] <- adapt_store_mse_mean
  result_matrix_mse
}
